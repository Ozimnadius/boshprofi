<!-- related -->
<section class="related">

    <h2 class="title related__title main-title">
        Сопутствующие товары
    </h2>

    <div class="container">
        <div class="row">
            <div class="related__prev">
                <button class="related__button">
                    <svg class="related__svg" viewBox="0 0 42.5 48.75">
                        <use xlink:href="images/icons/sprite.svg#slider_left"></use>
                    </svg>
                </button>
            </div>

            <div class="related__container">
                <div class="related__slide">
                    <!-- items -->
                    <div class="related__items">
                        <div class="related__items-title">
                            Топ-принадлежности
                        </div>
                        <div class="related__items-wrapper">
                            <div class="related__items-list">
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>

                            </div>
                        </div>
                    </div>
                    <!-- END items -->
                </div>
                <div class="related__slide">
                    <!-- items -->
                    <div class="related__items">
                        <div class="related__items-title">
                            Топ-принадлежности
                        </div>
                        <div class="related__items-wrapper">
                            <div class="related__items-list">
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>

                            </div>
                        </div>
                    </div>
                    <!-- END items -->
                </div>
                <div class="related__slide">
                    <!-- items -->
                    <div class="related__items">
                        <div class="related__items-title">
                            Топ-принадлежности
                        </div>
                        <div class="related__items-wrapper">
                            <div class="related__items-list">
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>

                            </div>
                        </div>
                    </div>
                    <!-- END items -->
                </div>
                <div class="related__slide">
                    <!-- items -->
                    <div class="related__items">
                        <div class="related__items-title">
                            Топ-принадлежности
                        </div>
                        <div class="related__items-wrapper">
                            <div class="related__items-list">
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>
                                <a href="product.php" class="related__item">
                                    <span class="related__item-title">
                                        Алмазные коронкиDiamond for Hard Ceramics
                                    </span>
                                </a>

                            </div>
                        </div>
                    </div>
                    <!-- END items -->
                </div>
            </div>

            <div class="related__next">
                <button class="related__button">
                    <svg class="related__svg" viewBox="0 0 42.5 48.75">
                        <use xlink:href="images/icons/sprite.svg#slider_right"></use>
                    </svg>
                </button>
            </div>
        </div>
    </div>

</section>
<!-- END related -->